...
- **3.2**: Chuyện gì sẽ xảy ra với giá trị của các biến toàn cục này ngay sau khi vi điều khiển thực hiện hành động Reset nếu hàm khởi tạo hệ thống (`Reset_Handler` / Startup code) quên nhiệm vụ sao chép dữ liệu từ `_sidata` đến `_sdata`? hợp Build & Flash
...
### Task 5 (Tự động hóa bằng Script): Xây dựng file Script tự động hóa tích
